...
namespace Samples.Presentation
{
	...
	public class DynamicMap_FeaturesPageViewModel : ViewModel, IDynamicMapComponent
	{
		...
		public Action<Windows.Devices.Geolocation.Geocoordinate> OnMapTapped
		{
			get => this.Get<Action<Windows.Devices.Geolocation.Geocoordinate>>();
			set => this.Set(value);
		}
		...
		private IDisposable ObserveUserLocation()
		{
			...
			async void UpdateLocationState((GeolocatorService.Geocoordinate location, bool isLocationEnabled) result)
			{
				if (result.isLocationEnabled)
				{
					...
					{
						var mapCenter = new Windows.Devices.Geolocation.Geopoint(new Windows.Devices.Geolocation.BasicGeoposition 
						{ 
							Latitude = result.location.Point.Position.Latitude, 
							Longitude = result.location.Point.Position.Longitude
						});
			...
			void UpdateSelectedPushpin(IGeoLocated[] selectedPushpins)
			{
				...
				else
				{
					SelectedPushpin = new PushpinEntity
					{
						...
						Coordinates = new Windows.Devices.Geolocation.Geopoint(new Windows.Devices.Geolocation.BasicGeoposition { Latitude = 0, Longitude = 0 })
					};
				}
			}
		}
		...
		{
			var mapViewPort = new MapViewPort(new Windows.Devices.Geolocation.Geopoint(new Windows.Devices.Geolocation.BasicGeoposition { Latitude = 45.504071, Longitude = -73.558709 }));
		...
		private PushpinEntity[] GetInitialPushpins()
		{
			return new[]
			{
					...
					{
						Name = "Pushpin 1",
						Coordinates = new Windows.Devices.Geolocation.Geopoint(new Windows.Devices.Geolocation.BasicGeoposition { Latitude = 46.3938717, Longitude = -72.0921769})
					},
					new PushpinEntity
					{
						Name = "Pushpin 2",
						Coordinates = new Windows.Devices.Geolocation.Geopoint (new Windows.Devices.Geolocation.BasicGeoposition { Latitude = 45.5502838, Longitude = -73.2801901 })
					},
					new PushpinEntity
					{
						Name = "Pushpin 3",
						Coordinates = new Windows.Devices.Geolocation.Geopoint(new Windows.Devices.Geolocation.BasicGeoposition { Latitude = 45.5502838, Longitude = -72.0921769 })
					},
				};
		}
		...
		public IDynamicCommand LocateMe => this.GetCommandFromTask(async ct =>
		{
			if (IsLocationEnabled)
			{
				try
				{
					...
					// We need to convert from geolocator.geopoint to windows because the service return type is not the one we need.
					var location = new Windows.Devices.Geolocation.Geopoint(new Windows.Devices.Geolocation.BasicGeoposition 
					{ 
						Latitude = currentLocation.Position.Latitude, 
						Longitude = currentLocation.Position.Longitude 
					});

					await OnLocateMeSuccess(ct, location);
		...
		private async Task OnLocateMeSuccess(CancellationToken ct, Windows.Devices.Geolocation.Geopoint location)
		{
		...
		private async Task<MapViewPort> ComputeMapViewPort(CancellationToken ct, Windows.Devices.Geolocation.Geopoint mapCenter = null)
		{
			var zoomLevel = ZoomLevels.District;

			if (mapCenter == default(Windows.Devices.Geolocation.Geopoint))
			{
		...
		{
			var defaultGeoPoint = new Windows.Devices.Geolocation.Geopoint(new Windows.Devices.Geolocation.BasicGeoposition { Latitude = 45.504071, Longitude = -73.558709 });
			try
			{
				var location = await _geolocatorService.GetLocationOrDefault(ct);
				var windowsGeoLocation = new Windows.Devices.Geolocation.Geopoint(new Windows.Devices.Geolocation.BasicGeoposition { Latitude = location.Latitude, Longitude = location.Longitude });
				return new GeoViewPort(windowsGeoLocation ?? defaultGeoPoint, ZoomLevels.District);
			}
			catch
			{
		...
		private async Task<PushpinEntity> CreatePushpinAtCenter(CancellationToken ct)
		{
			...
			{
				Coordinates = new Windows.Devices.Geolocation.Geopoint(new Windows.Devices.Geolocation.BasicGeoposition { Latitude = latitude, Longitude = longitude }),
				Name = string.Empty
			};
		}
		...
		public IDynamicCommand UpdateViewPort => this.GetCommand(() =>
		{
			...
			if (double.TryParse(latitudeString, out var latitude)
				...
				&& longitude <= 180)
			{
				var center = new Windows.Devices.Geolocation.Geopoint(new Windows.Devices.Geolocation.BasicGeoposition { Latitude = latitude, Longitude = longitude });
