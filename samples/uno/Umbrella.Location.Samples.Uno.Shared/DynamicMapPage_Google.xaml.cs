...
{
	public sealed partial class DynamicMapPage_Google : Page
	{
		public DynamicMapPage_Google()
		{
		...
		public class DynamicMapPageViewModel : PageViewModel
		{
			...
			private Task NavigateToDynamicMap_FeaturesPage(CancellationToken ct) => NavigateTo(ct, typeof(DynamicMap_FeaturesPage_Google));

			private Task NavigateToDynamicMap_MoveSearchPage(CancellationToken ct) => NavigateTo(ct, typeof(DynamicMap_MoveSearchPage_Google));

			private Task NavigateToDynamicMap_ZoomPoiPage(CancellationToken ct) => NavigateTo(ct, typeof(DynamicMap_ZoomPoiPage_Google));

			private Task NavigateToDynamicMap_SelectedFlipViewPage(CancellationToken ct) => NavigateTo(ct, typeof(DynamicMap_SelectedFlipViewPage_Google));

			private Task NavigateToPretty_PushpinSelectionPage(CancellationToken ct) => NavigateTo(ct, typeof(Pretty_PushpinSelectionPage_Google));
		}
	}
}
