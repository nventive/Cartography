	...
	public class DynamicMap_MoveSearchPageViewModel : ViewModel, IDynamicMapComponent
	{
		...
		private PushpinEntity[] GetPushpins(MapViewPortCoordinates boundaries)
		{
			return _allPushpins
				.Where(p =>
				{
					var pointPos = p.Coordinates.Position;
					var point = new Geocoordinate(pointPos.Latitude, pointPos.Longitude, DateTimeOffset.Now, new Geopoint(new BasicGeoposition(pointPos.Latitude, pointPos.Longitude)));
					return boundaries?.IsSurrounding(point) ?? false;
				})
				.ToArray();
		}
	}
