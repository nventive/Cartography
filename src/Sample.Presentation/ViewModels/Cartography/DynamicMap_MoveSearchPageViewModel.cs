...
namespace Sample.Presentation
{
	public class DynamicMap_MoveSearchPageViewModel : ViewModel, IDynamicMapComponent
    {
        ...
        {
            return _allPushpins
                .Where(p => boundaries?.IsSurrounding(new Geocoordinate
                (
                    p.Coordinates.Position.Latitude,
                    p.Coordinates.Position.Longitude,
                    DateTime.Now,
					new Geopoint(new BasicGeoposition{Latitude = 45.469967, Longitude = -73.591009})
				)) ?? false)
                .ToArray();
        }
    }
